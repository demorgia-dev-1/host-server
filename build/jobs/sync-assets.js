        ...
        try {
            ...
            for (const batchDir of batchDirs) {
                if (batchDir.isDirectory()) {
                    ...
                    if (fs_1.default.existsSync(path_1.default.join(baseDir, batchId, "evidences", "assessor", "adhar"))) {
                        const adharDir = path_1.default.join(baseDir, batchId, "evidences", "assessor", "adhar");
                        if (fs_1.default.existsSync(path_1.default.join(adharDir, "meta.json"))) {
                            ...
                            if (isNewFileUplaoded) {
                                ...
                                fs_1.default.writeFileSync(path_1.default.join(adharDir, "meta.json"), JSON.stringify(updateMetadata));
                                return;
                            }
                            for (const fileName of fileNames) {
                                ...
                                let url = null;
                                try {
                                    response = yield axios_1.default.get(`${process.env.MAIN_SERVER_URL}/assessor/offline-batches/${batchId}/sync-assessor-adhar?fileName=${fileName}&contentType=${mime_types_1.default.lookup(fileName)}`, {
                                        headers: {
                                    ...
                                    console.error(`❌ Error fetching presigned URL for file ${fileName} in batch ${batchId}:`, 
                                    // @ts-ignore
                                    error);
                                }
                                if (url) {
                            ...
                            fs_1.default.writeFileSync(path_1.default.join(adharDir, "meta.json"), JSON.stringify(metadata));
                        }
                    }
                                ...
                                if (fs_1.default.existsSync(path_1.default.join(practicalOnboardingDir, "meta.json"))) {
                                    ...
                                    for (const fileName of fileNames) {
                                        try {
                                            ...
                                            console.error("❌ Error syncing practical onboarding file:", error);
                                            continue;
                                        }
                                    }
                                }
                                    ...
                                    dir.split("/").pop(), "meta.json"))) {
                                        ...
                                        for (const fileName of fileNames) {
                                            try {
                                                ...
                                                console.error(`❌ Error syncing PMKY checklist file ${fileName} in batch ${batchId}:`, 
                                                // @ts-ignore
                                                error);
                                                continue;
                                            }
